# ============================================================
#  TEST akcji GRUPA pod SmartIT:
#  klik "Edit assignee" -> pole "Support group" -> przewin ->
#  wyczysc (Clear Support group) -> wpisz GLOBAL-SERVICEDESK -> wybierz
...
$win=Get-Edge; [Win]::SetForegroundWindow([IntPtr]$win.Current.NativeWindowHandle)|Out-Null; Start-Sleep 400
$edit=$null; foreach($e in $win.FindAll($TS::Descendants,$TRUE1)){ if($e.Current.ControlType.ProgrammaticName -notmatch 'Button|Hyperlink'){continue}; if((ToAscii $e.Current.Name).ToLower() -match 'edit assignee'){ $edit=$e; break } }
if($edit){ Write-Host "Klikam Edit assignee" -ForegroundColor Green; KEl $edit; Start-Sleep 1800 } else { Write-Host "Brak Edit assignee" -ForegroundColor Red }
$t=$null; foreach($e in $win.FindAll($TS::Descendants,$TRUE1)){ $ct=$e.Current.ControlType.ProgrammaticName; if($ct -notmatch 'ComboBox|Edit'){continue}; if((ToAscii $e.Current.Name).ToLower() -eq 'support group'){ $t=$e; break } }
...
$clr=$null; foreach($e in $win.FindAll($TS::Descendants,$TRUE1)){ if($e.Current.ControlType.ProgrammaticName -notmatch 'Button'){continue}; if((ToAscii $e.Current.Name).ToLower() -match 'clear support group'){ $clr=$e; break } }
if($clr){ Write-Host "Czyszcze (Clear Support group)"; KEl $clr; Start-Sleep 400; [Win]::Click($cx,$cy); Start-Sleep 400 }
