...
function Klik-EditAssignee($win){ $edit=Find1 $win $CTL::Button 'edit assignee'; if(-not $edit){ $edit=Find1 $win $CTL::Hyperlink 'edit assignee' }; if(-not $edit){ $edit=Find1 $win $CTL::Button 'edit' }; if($edit){ Write-Host ("   [edit] '"+$edit.Current.Name+"'") -ForegroundColor DarkGray; if(-not (Invoke-El $edit)){ Klik-El $edit|Out-Null }; Start-Sleep -Milliseconds 2000; return $true }; Write-Host "   [edit] nie znalazlem 'Edit assignee'" -ForegroundColor Red; return $false }
...
function Obsluz-Ostrzezenie($win){ Start-Sleep 500; foreach($n in @('^Yes$','^Tak$','^Continue$','^OK$')){ $b=Find1 $win $CTL::Button $n; if($b){ if(-not (Invoke-El $b)){ Klik-El $b|Out-Null }; Start-Sleep 700; return $true } }; return $false }
...
function Zapisz-Ticket($win){ foreach($n in @('^Save$','^Zapisz$','Save changes','Save and close')){ $b=Find1 $win $CTL::Button $n; if($b){ if(-not (Invoke-El $b)){ Klik-El $b|Out-Null }; return $true } }; return $false }
