...
function Front($win){ try{ $WSH.AppActivate([int]$win.Current.ProcessId) | Out-Null }catch{}; [Win]::SetForegroundWindow([IntPtr]$win.Current.NativeWindowHandle)|Out-Null; Start-Sleep -Milliseconds 400 }
...
function Wpisz-Combo($win,$target,$wartosc){ Front $win; Zapewnij-Widok $target; $fr=$target.Current.BoundingRectangle; $cx=[int]($fr.X+$fr.Width/2); $cy=[int]($fr.Y+$fr.Height/2); $sh=[System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Height; Write-Host ("   [pole y="+$cy+" / ekran do "+($sh-90)+"]") -ForegroundColor DarkGray; if($cy -lt 60 -or $cy -gt ($sh-90)){ Write-Host ("   [pole poza ekranem y="+$cy+" - NIE klikam]") -ForegroundColor Red; return $false }; Front $win; Klik-XY $cx $cy; Start-Sleep 400; [System.Windows.Forms.SendKeys]::SendWait('^a'); Start-Sleep 150; [System.Windows.Forms.SendKeys]::SendWait('{DELETE}'); Start-Sleep 200; [System.Windows.Forms.SendKeys]::SendWait((EscSK $wartosc)); Start-Sleep 1300; [System.Windows.Forms.SendKeys]::SendWait('{DOWN}'); Start-Sleep 300; [System.Windows.Forms.SendKeys]::SendWait('{ENTER}'); Start-Sleep 300; return $true }
...
function Obsluz-Ostrzezenie($win){ Start-Sleep 500; foreach($n in @('^Yes$','^Tak$','^Continue$','^OK$')){ $b=Find1 $win $CTL::Button $n; if($b){ Front $win; Klik-El $b|Out-Null; Start-Sleep 700; Write-Host "   [ostrzezenie] kliknieto '$($b.Current.Name)'" -ForegroundColor DarkGray; return $true } }; return $false }
