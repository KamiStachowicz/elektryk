...
function Wpisz-Combo($win,$target,$wartosc){ Front $win; Zapewnij-Widok $target; $fr=$target.Current.BoundingRectangle; $cx=[int]($fr.X+$fr.Width/2); $cy=[int]($fr.Y+$fr.Height/2); $sh=[System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Height; if($cy -lt 60 -or $cy -gt ($sh-90)){ Write-Host ("   [pole poza ekranem y="+$cy+" - NIE klikam]") -ForegroundColor Red; return $false }; Front $win; Klik-XY $cx $cy; Start-Sleep 400; [System.Windows.Forms.SendKeys]::SendWait('^a'); Start-Sleep 150; [System.Windows.Forms.SendKeys]::SendWait('{DELETE}'); Start-Sleep 200; [System.Windows.Forms.SendKeys]::SendWait((EscSK $wartosc)); Start-Sleep 1300; [System.Windows.Forms.SendKeys]::SendWait('{DOWN}'); Start-Sleep 300; [System.Windows.Forms.SendKeys]::SendWait('{ENTER}'); Start-Sleep 300; return $true }
...
while($stall -lt 4 -and $seen.Count -lt $MaxTicketow){
  ...
  if((-not $powrot) -and $juzPytano -and $PomijajJuzPytane){ $o=[System.Windows.Forms.MessageBox]::Show("Ticket "+$tkey+" - juz pytano/robione.`n`nTAK = pomin     NIE = przetworz mimo to     ANULUJ = STOP","Router - juz pytano",'YesNoCancel','Question'); if($o -eq 'Cancel'){ break }; if($o -eq 'Yes'){ $skipWaiting=$true } }
  $akcja=''
