...
function Klik-EditAssignee($win){ $edit=FindBtn $win 'edit assignee'; if(-not $edit){ $edit=FindBtn $win '^edit$' }; if(-not $edit){ $edit=Find1 $win $CTL::Hyperlink 'edit assignee' }; if($edit){ Write-Host ("   [edit] '"+$edit.Current.Name+"'") -ForegroundColor DarkGray; if(-not (Invoke-El $edit)){ Klik-El $edit|Out-Null }; Start-Sleep -Milliseconds 2000; return $true }; Write-Host "   [edit] nie znalazlem 'Edit assignee'" -ForegroundColor Red; return $false }
function Znajdz-PoleEx($win,$includes,$excludes){ for($i=0;$i -lt 3;$i++){ $cb=@($win.FindAll($TS::Descendants,(New-Object System.Windows.Automation.PropertyCondition($AE::ControlTypeProperty,$CTL::ComboBox)))); $ed=@($win.FindAll($TS::Descendants,(New-Object System.Windows.Automation.PropertyCondition($AE::ControlTypeProperty,$CTL::Edit)))); $all=$cb+$ed; foreach($inc in $includes){ foreach($e in $all){ $nm=(ToAscii $e.Current.Name).ToLower(); if($nm -match $inc){ $bad=$false; foreach($x in $excludes){ if($x -and ($nm -match $x)){ $bad=$true; break } }; if(-not $bad){ return $e } } } }; Start-Sleep -Milliseconds 700 }; return $null }
function Wpisz-Combo($win,$target,$wartosc){
  # bez fokusu: SetValue wprost w pole
  if(SetVal $target $wartosc){ Write-Host ("   [pole] SetValue '"+$wartosc+"'") -ForegroundColor DarkGray; Start-Sleep -Milliseconds 300; return $true }
  # fallback: klik + klawiatura (gdy SetValue nieobslugiwane)
...
function Akcja-Komentarz($win,$msg){ Front $win; $target=Find1 $win $CTL::Edit "New note"; if(-not $target){ Write-Host "   [komentarz] nie znalazlem pola 'New note'" -ForegroundColor Red; return $false }; Zapewnij-Widok $target; $done=$false; try{ $vp=$target.GetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern); $vp.SetValue($msg); $done=$true; Write-Host "   [komentarz] SetValue OK" -ForegroundColor DarkGray }catch{}; if(-not $done){ $r=$target.Current.BoundingRectangle; $shC=[System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Height; $cyC=[int]($r.Y+$r.Height/2); if($cyC -lt 60 -or $cyC -gt ($shC-90)){ Write-Host "   [komentarz poza ekranem - NIE klikam]" -ForegroundColor Red; return $false }; Front $win; [Win]::Click([int]($r.X+$r.Width/2),[int]($r.Y+$r.Height/2)); Start-Sleep 500; [System.Windows.Forms.SendKeys]::SendWait((EscSK $msg)); Start-Sleep 300 }; if($KomentarzPublic){ $cb=Find1 $win $CTL::CheckBox "Public"; if($cb){ try{ $tp=$cb.GetCurrentPattern([System.Windows.Automation.TogglePattern]::Pattern); if($tp.Current.ToggleState.ToString() -ne 'On'){ Klik-El $cb|Out-Null } }catch{ Klik-El $cb|Out-Null } } }; [console]::Beep(800,200); return $true }
function Obsluz-Ostrzezenie($win){ Start-Sleep 700; for($p=0;$p -lt 3;$p++){ $b=FindBtn $win '^(yes|tak|continue|ok)$'; if($b){ Write-Host ("   [ostrzezenie] '"+$b.Current.Name+"'") -ForegroundColor DarkGray; Front $win; try{ $b.SetFocus() }catch{}; Start-Sleep 200; Klik-El $b|Out-Null; Start-Sleep 250; [System.Windows.Forms.SendKeys]::SendWait('{ENTER}'); Start-Sleep 800; return $true }; Start-Sleep 500 }; return $false }
...
function Zapisz-Ticket($win){ $b=FindBtn $win '^(save|zapisz)$|save changes|save and close'; if($b){ Front $win; if(-not (Invoke-El $b)){ Klik-El $b|Out-Null }; return $true }; return $false }
