...
function Wpisz-Combo($win,$target,$wartosc){
  ...
  # ostatecznosc: pole poza ekranem - SetValue (moze sie nie zwiazac, sprawdz recznie)
  if(SetVal $target $wartosc){ Write-Host ("   [pole] poza ekranem - SetValue '"+$wartosc+"' (sprawdz czy sie zwiazalo)") -ForegroundColor Yellow; Start-Sleep 300; return $true }
  Write-Host "   [pole] nie udalo sie wpisac" -ForegroundColor Red; return $false }
