# ============================================================
#  SAP TICKET ROUTER - PELNY (czytanie + akcja GSD)
#  - przechodzi tickety, czyta, decyduje
#  - dla decyzji GSD: Edit -> pole grupy -> GLOBAL-SERVICEDESK -> STOP
#    (pauza: Ty zapisujesz recznie, potem ENTER -> bot leci dalej)
#  - NASZE / DO_SPRAWDZENIA: tylko log
...
$GRUPA          = "GLOBAL-SERVICEDESK"        # co wpisujemy dla GSD
...
function Przetworz($txt){
 # regula: shopfloor -> GSD
 if($txt -match 'shopfloor'){ return [pscustomobject]@{ System='SHOPFLOOR'; Decyzja='GSD'; Role=@(); UserName=''; UserId='' } }
...
function Get-RowKey($btn){ $node=$btn; for($k=0;$k -lt 8;$k++){ $p=$WALK.GetParent($node); if(-not $p){ break }; $node=$p; $rr=$node.Current.BoundingRectangle; if($rr.Width -ge 400 -and $rr.Height -le 140){ break } }; $t=''; foreach($d in $node.FindAll($TS::Descendants,$TRUE1)){ $nm=$d.Current.Name; if($nm){ $t+=$nm+'|' } }; return ($t -replace '\s+',' ').Trim() }
...
# AKCJA GSD: Edit -> pole -> GLOBAL-SERVICEDESK (nie zapisuje). Zwraca $true jak wypelnil.
function Akcja-GSD($win){
...
# --- Start ---
Clear-Host
Write-Host "SAP Ticket Router - PELNY (czytanie + akcja GSD)" -ForegroundColor Cyan
...
while($stall -lt 4 -and $seen.Count -lt $MaxTicketow){
  ...
  Write-Host ("--- Ticket #"+$nr+"  SYSTEM="+$w.System+"  DECYZJA="+$w.Decyzja+$(if($osoba){"  -> "+$osoba}else{''})) -ForegroundColor $kol
  ...
  if($w.Decyzja -eq 'GSD'){
    Write-Host "   -> AKCJA: ustawiam GLOBAL-SERVICEDESK (nie zapisuje)" -ForegroundColor Yellow
    $win=Get-EdgeWindow
    if(Akcja-GSD $win){ Read-Host "   >>> SPRAWDZ i ZAPISZ ticket w Edge, potem wroc tu i nacisnij ENTER" }
  }
